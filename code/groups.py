...
class AllSprites(pygame.sprite.Group):
    def __init__(self, width, height, clouds, horizon_line, bg_tile = None, top_limit = 0):
        ...
        if bg_tile:
            ...
            # small cloud
            self.cloud_timer = Timer(randint(2000, 5000), self.create_cloud, True)
            self.cloud_timer.activate()
            for cloud in range(10):
                pos = (randint(0, self.width), randint(self.borders['top'], self.horizon_line - 100))
                Surf = choice(self.small_cloud)
                Cloud(pos, Surf, self , randint(20, 70))
    ...
    def create_cloud(self):
        pos = (randint(self.width + 500, self.width + 600), randint(self.borders['top'], self.horizon_line - 100))
        Surf = choice(self.small_cloud)
        Cloud(pos, Surf, self , randint(20, 70))
